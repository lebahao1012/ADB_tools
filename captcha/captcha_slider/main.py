...
class PuzleSolver:
    ...
    def __background_preprocessing(self, y_sup, y_inf):
        ...
        background = self.__background_to_grayscale(background)
    ...
    def __piece_preprocessing(self):
        ...
        template = self.__piece_to_grayscale_(template)
        print("x", x)
        print("y", y)
        print("w", w)
        print("h", h)
    ...
    def __background_to_grayscale(self, img):
        tmp_path = "captcha_slider/background_sobel.png"
        cv2.imwrite(tmp_path, img)
        return cv2.imread(tmp_path, 0)
    
    def __piece_to_grayscale_(self, img):
        tmp_path = "captcha_slider/piece_sobel.png"
        cv2.imwrite(tmp_path, img)
        return cv2.imread(tmp_path, 0)
    
# solver = PuzleSolver("captcha_slider/piece.png", "captcha_slider/background.png")
